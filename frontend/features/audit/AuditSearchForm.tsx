      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-12 lg:gap-20 items-start">
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="flex flex-col gap-3">
            ...
            <p className="text-sm leading-relaxed max-w-[42ch]" style={{ color: "#5a5a5a" }}>
          ...
          <form onSubmit={submit} className="flex flex-col gap-4">
            ...
            >
              ...
              <input
                ...
                className="flex-1 bg-transparent text-sm focus:outline-none placeholder:text-[#3a3a3a]"
                style={{ color: "#eee" }}
                autoFocus
              />
              {query && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <FieldBox label="SIREN (optionnel)">
                <input
                  ...
                  className="w-full bg-transparent text-sm focus:outline-none placeholder:text-[#3a3a3a]"
                  style={{ color: "#eee" }}
                />
              </FieldBox>
              <FieldBox label="Secteur">
                ...
                >
                  {[
                    ...
                    <option key={v} value={v} style={{ background: "#181818" }}>{l}</option>
                  ))}
                </select>
              </FieldBox>
            </div>
        ...
        >
          <p className="text-xs font-medium tracking-widest uppercase" style={{ color: "#3a3a3a" }}>
            Ce qu'Unmask vérifie
          </p>
          <ul className="flex flex-col divide-y" style={{ borderColor: "#1e1e1e" }}>
            {PILLARS.map(({ num, label }) => (
              <li key={num} className="flex items-center gap-4 py-3">
                ...
                <span className="text-sm" style={{ color: "#8a8a8a" }}>{label}</span>
              </li>
            ))}
          </ul>
...
function FieldBox({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="flex flex-col gap-1.5">
      <span className="text-[10px] font-medium uppercase tracking-widest" style={{ color: "#3a3a3a" }}>
        {label}
      </span>
      <div
